// Cobro — pantalla del empleado: iniciar turno, cobrar y cerrar caja.
import { useState, useEffect, useMemo } from 'react';
import { Link } from 'react-router-dom';
import { useStore } from '../lib/store.js';
import { useTheme, ThemeStyle, withAliases, THEMES } from '../lib/theme.jsx';
import { Icon } from '../lib/icons.jsx';
import { PAY_METHODS } from '../lib/constants.js';
import { fmt, uid, initials, fmtTime } from '../lib/format.js';
import SyncBadge from '../components/SyncBadge.jsx';

let TH = withAliases(THEMES.flow);
...
export default function Cobro() {
  ...
  const totals = useMemo(() => {
    ...
    return openShift.entries.reduce(
      (acc, e) => {
        acc.total += e.price;
        acc.emp += e.price * pct;
  ...
  function startShift() {
    setState((s) => ({
      ...
      openShifts: {
        ...(s.openShifts || {}),
        [empId]: { id: uid('sh'), empId, startTime: new Date().toISOString(), entries: [] },
      },
    }));
  }
  ...
  function closeShift() {
    ...
    setState((s) => {
      const { [empId]: _omit, ...rest } = s.openShifts || {};
      ...
      <div style={{ minHeight: '100vh' }}>
        ...
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
          <div style={{ textAlign: 'center', maxWidth: 400 }}>
            ...
            <Link to="/admin" style={{ color: TH.gold }}>Abrir panel admin →</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      <ThemeStyle TH={TH} />
      <Topbar
        employee={employee} employees={activeEmps} setEmpId={setEmpId}
        openShifts={state.openShifts || {}} openShift={openShift} totals={totals}
        syncStatus={syncStatus} onCloseTurn={() => setShowSummary(true)}
      />
      ...
      {showSummary && (
        <SummaryModal
          entries={openShift?.entries || []} totals={totals} employee={employee} approvalCode={state.approvalCode}
...
function Topbar({ employee, employees, setEmpId, openShifts, openShift, totals, syncStatus, onCloseTurn }) {
  return (
    <div
      style={{
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: TH.surface, border: `1px solid ${TH.line}`, borderRadius: 999, padding: '4px 4px 4px 14px' }}>
        ...
        <select
          ...
          disabled={!!openShift}
          style={{
            background: 'transparent', border: 'none', color: TH.text, fontSize: 14, fontWeight: 500,
            padding: '8px 10px', cursor: openShift ? 'not-allowed' : 'pointer', outline: 'none', opacity: openShift ? 0.6 : 1,
          }}
        >
          {employees.map((e) => {
            const has = !!openShifts[e.id];
            return (
              <option key={e.id} value={e.id} style={{ background: TH.surface }}>
                {e.name}{has ? ' · turno abierto' : ''}
              </option>
            );
          })}
        </select>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <Link to="/" style={{ color: TH.textDim, fontSize: 13 }}>Salir</Link>
      </div>
    </div>
  );
}
      ...
      <div style={{ width: '100%', maxWidth: 480, textAlign: 'center' }}>
        <div
          style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 28px', position: 'relative',
          }}
        >
        ...
        <h1 className="th-title" style={{ fontSize: 36, fontWeight: 700, margin: '0 0 10px', color: TH.accent, letterSpacing: '0.04em' }}>
...
function PaymentSheet({ svc, onCancel, onConfirm }) {
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}
      onClick={onCancel}
    >
      <div onClick={(e) => e.stopPropagation()} style={{ background: TH.surface, borderRadius: 20, border: `1px solid ${TH.line}`, width: '100%', maxWidth: 520, padding: 28 }}>
          ...
          {PAY_METHODS.map((m) => (
            <button
              ...
              style={{
                ...
                display: 'flex', alignItems: 'center', gap: 12, padding: '0 18px', cursor: 'pointer', color: TH.text, transition: 'all 100ms',
              }}
        ...
        <button
          onClick={onCancel}
          style={{ marginTop: 14, width: '100%', height: 48, borderRadius: 12, background: 'transparent', border: `1px solid ${TH.line}`, color: TH.textDim, fontSize: 14, cursor: 'pointer', fontWeight: 500 }}
        >
          Cancelar
        </button>
      </div>
...
function SummaryModal({ entries, totals, employee, approvalCode, startTime, onClose, onRemove, onConfirmClose }) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}
      onClick={onClose}
    >
      <div onClick={(e) => e.stopPropagation()} style={{ background: TH.surface, borderRadius: 20, border: `1px solid ${TH.line}`, width: '100%', maxWidth: 720, maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
        ...
        <div style={{ padding: 28 }}>
          <div style={{ background: `linear-gradient(135deg, ${TH.gold}26, ${TH.gold}0d)`, border: `1px solid ${TH.gold}66`, borderRadius: 18, padding: 24, marginBottom: 16, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 12, color: TH.textDim, marginTop: 6 }}>
              {employee.commissionPct}% sobre {fmt(totals.total)} cobrado · {entries.length} servicios
            </div>
          </div>
  ...
  return (
    <div onClick={onCancel} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 20, zIndex: 5 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: TH.surface, borderRadius: 16, border: `1px solid ${TH.line}`, padding: 28, maxWidth: 360, width: '90%', textAlign: 'center' }}>
        ...
        <input
          autoFocus type="password" maxLength="4" value={code}
