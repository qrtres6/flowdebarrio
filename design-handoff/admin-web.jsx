// admin-web.jsx — Web admin panel, Android Material dark style
const { useState, useEffect, useMemo } = React;

let TH = THEMES.barberia;
// Backwards-compat aliases (legacy property names used throughout this file)
function syncTH(theme) {
  TH = { ...theme, gold: theme.accent };
}
syncTH(THEMES[loadTheme()] || THEMES.barberia);
...
function fmtDay(iso) { return new Date(iso).toLocaleDateString('es-AR', { day: '2-digit', month: 'short' }); }
...
function App() {
  ...
  if (!authed) {
    return <AdminLogin TH={TH} adminPassword={state.adminPassword || 'flow2026'} onOk={() => {
      try { sessionStorage.setItem('flow.admin.authed', '1'); } catch {}
      setAuthed(true);
    }}/>;
  }
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <ThemeStyle TH={TH}/>
      <Sidebar tab={tab} setTab={setTab} shopName={state.shopName} />
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <Topbar tab={tab} shopName={state.shopName} onLogout={() => {
          try { sessionStorage.removeItem('flow.admin.authed'); } catch {}
          setAuthed(false);
        }} />
...
function Sidebar({ tab, setTab, shopName }) {
  ...
  return (
    <aside style={{
      ...
      padding: '20px 12px', display: 'flex', flexDirection: 'column', gap: 4, position: 'sticky', top: 0, height: '100vh', alignSelf: 'flex-start',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px 18px' }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          <div className="th-title" style={{ fontSize: 14, fontWeight: 700, color: TH.accent, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', letterSpacing: '0.06em' }}>Flow de Barrio</div>
        </div>
      </div>
      {items.map(it => {
      ...
      <div style={{ marginTop: 'auto', padding: '12px', borderTop: `1px solid ${TH.line}`, display: 'flex', alignItems: 'center', gap: 10 }}>
...
// ─── Topbar ───
function Topbar({ tab, shopName, onLogout }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <a href="cobro.html" style={{ height: 40, padding: '0 18px', borderRadius: 999, background: 'transparent', border: `1px solid ${TH.line}`, color: TH.text, display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 500, textDecoration: 'none' }}>
...
function aggregateShifts(shifts, services, employees) {
...
// ─── Panel (resumen con ganancia neta) ───
function Panel({ state, period, setPeriod }) {
  ...
  const agg = useMemo(() => aggregateShifts(shifts, state.services, state.employees), [shifts, state.services, state.employees]);
  const totalExpenses = expenses.reduce((a, e) => a + Number(e.amount), 0);
  const ganancia = agg.totalShopCut - totalExpenses;

  const periodLabel = { today: 'hoy', week: 'esta semana', month: 'este mes', all: 'histórico' }[period];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        ...
        <Card pad={20}>
          ...
          <div style={{ fontSize: 28, fontWeight: 500, marginTop: 14, letterSpacing: -0.5 }}>
            {state.employees.filter(e => e.active).length}<span style={{ color: TH.textMute, fontSize: 18 }}> / {state.employees.length}</span>
          </div>
      ...
      {/* Empleados ranking + por método de pago */}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Card pad={24}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: TH.textDim, marginTop: 4 }}>{expenses.length} gasto{expenses.length === 1 ? '' : 's'} registrado{expenses.length === 1 ? '' : 's'}</div>
          </div>
          {cats.length > 0 && (
            ...
            <tbody>
              {[...expenses].sort((a, b) => new Date(b.date) - new Date(a.date)).map(e => (
                <tr key={e.id} style={{ borderBottom: `1px solid ${TH.line}` }}>
                  <Td><span style={{ fontFamily: 'Roboto Mono', fontSize: 13, color: TH.textDim }}>{fmtDay(e.date)}</span></Td>
                  <Td>
      ...
      {creating && (
        <ExpenseModal
          ...
          isNew onSave={create} onCancel={() => setCreating(false)}
        />
      )}
    </div>
  );
...
// ─── Servicios (sin pct — ahora la comisión viene del empleado) ───
  ...
  return (
    <Modal title={isNew ? 'Nuevo servicio' : 'Editar servicio'} onClose={onCancel}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
        ...
        <div style={{ flex: 1 }}/>
        <button onClick={onCancel} style={{ ...btnStyle, background: 'transparent', border: `1px solid ${TH.line}`, color: TH.text }}>Cancelar</button>
        <button onClick={() => onSave({ ...svc, name, price, icon })}
          style={{ ...btnStyle, background: TH.gold, color: '#0E0E10', border: 'none' }}>
...
function Empleados({ state, setState, notify }) {
  ...
  return (
    <div>
          ...
          <tbody>
            {state.employees.map(e => (
              <tr key={e.id} style={{ borderBottom: `1px solid ${TH.line}` }}>
                ...
                <Td>
                  <button onClick={() => toggleActive(e.id)} style={{
      ...
      {creating && <EmployeeModal emp={{ name: '', role: 'Barbero', active: true, startDate: new Date().toISOString().slice(0,10), color: '#B8893E', commissionPct: 50 }} isNew onSave={create} onCancel={() => setCreating(false)} />}
    </div>
  );
}
  ...
  return (
    <Modal title={isNew ? 'Registrar empleado' : 'Editar empleado'} onClose={onCancel}>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
            {colors.map(c => (
              <button key={c} onClick={() => setColor(c)} style={{
                ...
                border: c === color ? `2px solid ${TH.text}` : '2px solid transparent',
              }}/>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
        ...
        <div style={{ flex: 1 }}/>
        <button onClick={onCancel} style={{ ...btnStyle, background: 'transparent', border: `1px solid ${TH.line}`, color: TH.text }}>Cancelar</button>
        <button onClick={() => onSave({ ...emp, name, role, active, startDate, color, commissionPct: pct })}
          style={{ ...btnStyle, background: TH.gold, color: '#0E0E10', border: 'none' }}>
      ...
      <Card pad={24}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
          {Object.values(THEMES).map(t => {
            ...
            return (
              <button key={t.id} onClick={() => { setTheme(t.id); notify(`Tema "${t.name}"`); }} style={{
...
function Switch({ value, onChange, onLabel = 'Sí', offLabel = 'No' }) {
  return (
    <button onClick={() => onChange(!value)} style={{
      display: 'flex', alignItems: 'center', gap: 12, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0,
    }}>
      <div style={{
        width: 48, height: 28, borderRadius: 999, position: 'relative',
        background: value ? TH.gold : TH.surface3, border: `1px solid ${value ? TH.gold : TH.line}`, transition: 'all 150ms',
      }}>
        <div style={{
          position: 'absolute', top: 2, left: value ? 22 : 2,
          width: 22, height: 22, borderRadius: 999, background: value ? '#0E0E10' : TH.textDim, transition: 'left 150ms',
        }}/>
      </div>
...
function FabBtn({ icon, label, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: `0 8px 24px ${TH.gold}44`,
      whiteSpace: 'nowrap',
    }}>
...
function Snackbar({ msg, kind }) {
  ...
  return (
    <div style={{
      ...
      animation: 'snackUp 200ms ease-out',
...
function AdminLogin({ TH, adminPassword, onOk }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: TH.bg }}>
      ...
      <form onSubmit={submit} style={{
        ...
        <input type="password" autoFocus value={pwd} onChange={e => setPwd(e.target.value)} placeholder="Contraseña"
          style={{
        ...
        <a href="cobro.html" style={{ fontSize: 12, color: TH.textDim, textDecoration: 'none' }}>← Volver al cobro</a>
      </form>
    </div>
  );
}
